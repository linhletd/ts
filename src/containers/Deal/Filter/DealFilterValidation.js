...
export default function DealFilterValidation() {
  ...
  return useMemo(() => {
    return yup.object({
      ...
      // }),
      // startTime: yup.number().typeError().default(0).notRequired(),
      // endTime: yup.number().typeError().notRequired().moreThan(yup.ref('startTime'), t('form.validate.deal.endTime.m2')),
